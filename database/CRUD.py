...
class MongoDBHandler:
    ...
    def insertDocument(self, collection_name: str, data: dict) -> Result:
        try:
            ...
            return Result(Data={"Id":data.inserted_id},Status=1, Message=message)
        
        except DuplicateKeyError as dup_ex:
            message = (
                f"Duplicate key error inserting document into {collection_name}: {dup_ex}"
            )
    ...
    def insertDocuments(self, collectionName: str, dataList: list) -> Result:
        try:
            ...
            for error in bwe.details['writeErrors']:
                errorMessages.append(f"Error at index {error['index']}: {error['errmsg']}")
    ...
    def delete_document(self, collection_name: str, query: dict) -> Result:
        try:
            collection = self.db[collection_name]
